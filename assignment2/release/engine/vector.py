...
class Vec2d(object):
    ...
    def getAverage(vectors):
        ...
        for vec in vectors:
            sumX += vec.x
            sumY += vec.y
        aveX = sumX / len(vectors)
        aveY = sumY / len(vectors)
        return Vec2d(aveX, aveY)
    ...
    def project(point, vector):
        return point.dot(vector) / vector.dot(vector)

    @staticmethod
    def projectPoints(points, vector):
        values = []
        for point in points:
            value = Vec2d.project(point, vector)
            values.append(value)
